...
fn dissassemblePacket (y: &[u8]) -> ~[~[f32]] {
	let mode_a: u8 = y[0];
	let mode_b: u8 = y[1];
	let flags: u8 = y[2];
	...
	match c.find_by_vid_pid(0x59e3, 0xcee1) {
		...
		match dev.open() {
			Ok(handle) => {
				...
				let ho = handle.clone();
				do spawn {
					ho.write_stream(0x02, usb::libusb::LIBUSB_TRANSFER_TYPE_BULK, 32, 6, |buf| {
						let mut y = buf.unwrap();
						assemblePacket(y);
						true
					});
				}
